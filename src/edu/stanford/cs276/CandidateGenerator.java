...
public class CandidateGenerator implements Serializable {
  ...
  */
  ...
  public Set<String> getCandidates(String query, LanguageModel languageModel,
    ...
    // no space operation
    for (Pair<String,Integer> pair : queryDel1.get(0)) {
      move1AddCandidate(pair.getFirst(), candidates, dict, 2);
    }
    for (Pair<String,Integer> pair : queryDel1.get(1)) {
      error1Move1AddCandidate(pair.getFirst(), candidates, dict, pair.getSecond());
    }
    for (Pair<String,Integer> pair : queryIns1.get(0)) {
      move1AddCandidate(pair.getFirst(), candidates, dict, 2);
    }
    ...
    // space operation
    for (Pair<String,Integer> pair : queryDel1.get(1)) {
      move1AddCandidate(pair.getFirst(), candidates, dict, 1);
    }
    for (Pair<String,Integer> pair : queryIns1.get(1)) {
      move1AddCandidate(pair.getFirst(), candidates, dict, 1);
    }
    ...
    for (Pair<String,Integer> pair : queryDel1.get(2)) {
      error2Move1AddCandidate(pair.getFirst(), candidates, dict, pair.getSecond());
    }
    ...
    coun++;
    System.err.println(coun + " " + candidates.size());
